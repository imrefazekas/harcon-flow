	...
	generateDefs: function ( flowDefs, opts = {}, callback ) {
		...
		return new Promise( (resolve, reject) => {
			...
			Promise.all( flowDefs.map( function (flowDef) {
				return self.parse( flowDef.name || flowDef.title, flowDef.def )
			} ) )
			.then( (flows) => {
				flows.forEach( function (flow) {
					flow.messages.forEach( function (message) {
						...
						let flowLink = flows.find( function (flow) { return flow.title === message.link } )
	...
	parse: function ( title, flowDef, callback ) {
		return new Promise( (resolve, reject) => {
			...
			try {
				...
				return Proback.resolver( { title: title, source: source, messages: messages}, callback, resolve)
