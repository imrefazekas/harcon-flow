...
module.exports = {
	valid: function ( flowDefs, callback ) {
		var self = this
		return new Promise( (resolve, reject) => {
			self.generateDefs( flowDefs, {} )
			.then( (flows) => {
				Proback.resolver( { valid: true, flows: flows }, callback, resolve )
			} )
			.catch( (reason) => {
				Proback.resolver( { valid: false, error: reason }, callback, resolve )
			} )
		} )
	},
	generateDefs: function ( flowDefs, opts = {}, callback ) {
		var self = this
		return new Promise( (resolve, reject) => {
			var res = { }
			Promise.all( flowDefs.map( function (flowDef) {
				return self.parse( flowDef.name || flowDef.title, flowDef.def, flowDef.validation )
			} ) )
			.then( (flows) => {
				flows.forEach( function (flow) {
					flow.messages.forEach( function (message) {
	...
	parse: function ( title, flowDef, validation = {}, callback ) {
		return new Promise( (resolve, reject) => {
			...
			try {
				...
				return Proback.resolver( { title: title, source: source, messages: messages, validation: validation }, callback, resolve)
